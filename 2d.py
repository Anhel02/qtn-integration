...
def integrand(X, Y, eps=1e-30):
    return np.sin(1/(X+eps)+1/(Y+eps))**2
...
def run(n, rel_acc, shots, x_min, x_max, y_min, y_max, transpile, show_qc, show_specs):
    V = (x_max - x_min)*(y_max - y_min) # Integration volume
    ...
    n_per_q = int(n / 2)
    x_q = quantization(X, n_per_q) # Midrise quantization
    y_q = quantization(Y, n_per_q)
...
if __name__ == '__main__':
    ...
    rel_acc = 0.1 # relative accuracy (TT_SVD truncation) [0: No Truncation]
    shots = 1000
    x_min = 10.
    x_max = 100.
    y_min = 10.
    ...
    estimator, std = run(n, rel_acc, shots, x_min, x_max, y_min, y_max, transpile, show_qc, show_specs)
    analytical, analytical_std = dblquad(lambda y, x: integrand(x, y), x_min, x_max, y_min, y_max)
