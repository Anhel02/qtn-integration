...
class Bubble:
    ...
    def _deformed_loop_mom(self, z, eta=0.4, width=15.0):
        l_real = self.m * z / (1 - z)
        dl_dz = self.m / (1 - z)**2

        l_sing = np.sqrt((self.p0 / 2)**2 - self.m**2)
        ...
        J_complex = (1 - 1j * dy_dl) * dl_dz

        return l_complex, J_complex


    def integrand(self, z):
        z = np.clip(z, 0., 1.-1e-9) # z in [0,1)

        if self.p0 >= 2 * self.m:
            l, J = self._deformed_loop_mom(z)
        else:
            l, J = self._loopMom(z)

        # on-shell energies
        q_plus = np.sqrt(l**2 + self.m**2 - self.i0)
        q_UV_plus = np.sqrt(l**2 + self.mu_UV**2 - self.i0)

        # causal propagators
        lambda_plus = 2 * q_plus + self.p0
        lambda_minus = 2 * q_plus - self.p0
        x2 =  (2 * q_plus)**2

        phase_space = (l**2) / (2 * np.pi**2)

        # integrand
        I = phase_space * (1/x2 * (1/lambda_plus + 1/lambda_minus) - 1/(4 * q_UV_plus**3))
...
def run(diagram, n, rel_acc, shots, x_min, x_max, transpile, show_qc, show_specs):
    V = x_max - x_min # Integration volume
 
    # Original distribution
    X = np.linspace(x_min,x_max,1000)
    Y = diagram.integrand(X)

    # Quantized distribution
    z = quantization(X,n) # Midrise quantization
    f = diagram.integrand(z)
    ...
    var_real = 1 / (shots - 1) * (V**2 * np.sum(p_q * (f.real * w)**2) - estimator.real**2)
    var_imag = 1 / (shots - 1) * (V**2 * np.sum(p_q * (f.imag * w)**2) - estimator.imag**2)
...
if __name__ == '__main__':
    ...
    rel_acc = 0.001# relative accuracy (TT_SVD truncation) [0: No Truncation]
    shots = 1000
    x_min = 0.
    x_max = 1.
    ...
    diagram = Bubble(m, mu_UV, p0)

    estimator, std = run(diagram, n, rel_acc, shots, x_min, x_max, transpile, show_qc, show_specs)
